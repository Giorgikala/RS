...
namespace LastProject.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        {       
            ...
            messy.From = new MailAddress("giorgikalanadadze24@gmail.com");
            ...
            smt.Credentials = new NetworkCredential("giorgikalanadadze24@gmail.com", "sreoovjvsewhnocv");
            try
            {
                smt.Send(messy);
        ...
        public async Task <ActionResult<Token>> login(LoginDto request)
        {
            ...
            if (user is null)
            {
                ...
                if (doctor is null)
                    return BadRequest("doctor cant be found");

                if (!BCrypt.Net.BCrypt.Verify(request.Password, doctor.PasswordHash))
                {
                    return BadRequest("wrong password");
                }
                string toke = CreateToken(request, "doctor");
                token.MyToken = toke;
            ...
            string Token = CreateToken(request, "user");
        ...
        ///CreateToken method
        private string CreateToken(LoginDto loginDto, string role)
        {
